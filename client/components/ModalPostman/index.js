              ...
              return item.type === 'dataSource' ?
                <Col span={8} className="modal-postman-col" key={index}>
                  <Collapse className="modal-postman-collapse" defaultActiveKey={['1']} bordered={false} accordion>
                    ...
                    {
                      envType === 'case' && <Panel
                        header={<h3 className="mock-title">变量&nbsp;<Tooltip placement="top" title="Mock提供了强大的变量参数功能，你可以在测试的时候使用前面接口的 参数 或 返回值 作为 后面接口的参数，即使接口之间存在依赖，也可以轻松 一键测试~"><Icon type="question-circle-o" /></Tooltip></h3>}
                        key="3">
